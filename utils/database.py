    ...
    def update_message_status(self, message_id, new_status):
        ...
        with self.get_connection() as conn:
            ...
            try:
                ...
                # 1. Try direct message_id update first
                cursor.execute('''
                    UPDATE messages
                    SET status = ?
                    WHERE message_id = ?
                ...
                if cursor.rowcount > 0:
                    updated = True
                    print(f"DEBUG: Updated message by message_id: {message_id}")

                # 2. If that fails and message_id is an integer, try direct ID update
                ...
                # 3. If message_id is a string (UUID), try using the database ID
                if not updated and isinstance(message_id, str):
                    # Get matching message by content or other fields
                    cursor.execute('''
                        UPDATE messages
                    ...
                    if cursor.rowcount > 0:
                        updated = True
                        print(f"DEBUG: Updated most recent message and assigned message_id: {message_id}")
