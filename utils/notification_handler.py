import wx
import wx.adv  # Add this import for advanced controls
import platform
import os
...
class NotificationHandler:
    ...
    def _show_wx_notification(self, title, message):
        ...
        try:
            # Try using wx.adv.NotificationMessage (newer wxPython)
            ...
            try:
                # Try using a simple message dialog as fallback
                app = wx.GetApp()
                if app and app.IsMainLoopRunning():
                    wx.CallAfter(self._show_dialog_notification, title, message)
                else:
                    print(f"NOTIFICATION: {title} - {message}")
            except Exception as e2:
                print(f"Failed to show notification: {e2}")
                print(f"NOTIFICATION: {title} - {message}")

    def _show_dialog_notification(self, title, message):
        """Show a simple dialog notification"""
        frame = wx.Frame(None, -1, "")
        frame.Centre()

        # Create a popup dialog
        info = wx.adv.NotificationMessage(title, message)
        info.Show(timeout=5)  # 5 seconds

        # Clean up the frame after notification
        wx.CallLater(5000, frame.Destroy)
